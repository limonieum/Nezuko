...
fetch(`https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(message.content)}&botname=${client.user.username}&ownername=Saputra`)
    .then(res => res.json())
    .then(data => {
...
client.on("ready", () => {
    ...
    console.log("automodv12 beta is ready join support server https://discord.gg/yQGBA9q3tx")
});
client.on("message", async message => {
if(!message.guild) return;
...
console.log("Subscribe on Content Tv")
...
client.on("message", async message => {
  if (message.channel.name == "abotchat") {
    ...
    message.channel.startTyping();
    if (!message.content) return message.channel.send("Please say something.");
    scb.chat({message: message.content, name: client.user.username, owner:"Saputra", user: message.author.id, language:"auto"}).then(reply => {
